...
if ($handoff) {
  ...
  @{ phase='verified'; target=$handoff.target; expectedVersion=$Expect; artifactSha256=$actualArtifact; completedAt=[DateTimeOffset]::UtcNow.ToString('o'); ownerPid=$PID; restoration=$restoration; healthBuild=$h.build; beforeInventory=$inventoryPath } |
