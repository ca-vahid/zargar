...
### 2.4 The durable position manager — holding for days or weeks *(built 2026-08-27; the text below is the spec it was built to, plus §9's A-items)*
...
### 2.5 What "iron clad" means as tests *(the suite exists: `tests/test_position_chaos.py`, 14 scenarios, all green)*
...
| **5 · second technique** ✅ *SHIPPED (2026-08-27/28)* | **Tip** (`zargar/techniques/tip/` — plan/horizon/express/runner; dual shadow books, expiry-bounded waiting, 2b handoff, options expression in both books, redesigned Tips page) and **Flow** (`zargar/techniques/flow/` — nightly scan on `engine.scheduler` reading `option_chain_snapshots`, `FlowContextServed` deliveries, universe flow layer, full Flow page: Reads desk · Symbol Story · Brief). Gaps found became platform fixes, never forks: the orders.py bracket-child deadlock, technique-scoped restore, the tracker volume opt-out, `ArmConfig.premium_budget` | new feature | done |
