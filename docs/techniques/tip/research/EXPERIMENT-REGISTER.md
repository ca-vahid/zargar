...
- **Costs:** $35 hard ceiling (`review_frozen.SuiteBudget` - one durable write-ahead ledger across models, cases, turns, retries and invocations; conservative reservation; holds while no attempt bills > 1.25x its reservation), 60 stratified cases (20 management / 10 missed-entry / 8 correction / 6 mixed / 16 note-only), one pass per model; plan `economics/review-model-evaluation-plan.md` (`tip_review_gate_eval --model-plan`). Candidate list prices live in the tool, not in `llm.rates`; re-verify on the approval day.
