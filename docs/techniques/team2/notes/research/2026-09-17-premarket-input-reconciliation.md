...
|---|---|---|---|---|
| SPY | PML **660.65** | 757.53 | 07:46 | an exchange **correction** delivered at 08:02:09.99 ET replaced the 07:46 bar 760.21/760.21/760.21/760.21/615 with 760.12/760.2369/**660.65**/760.2285/615 (same volume, low 100 points under the open; a dropped digit is a hypothesis). The bank holds the ORIGINAL bar. |
...
Provenance of the corrections — **UNRESOLVED** (corrected after the PR #204 review): every `bar_revised` row says `source exchange`
on both sides, and that label does not identify the producer. In hybrid mode `engine.py` wires BOTH feeds to `_ingest_exchange_bars`:
Alpaca's stream and `YahooQuoteFeed`, which repeatedly requests chart data with `includePrePost=true`, parses completed bars and hands
them to `on_bars` stamped `exchange`. Arrival time and the float32-shaped values of the later QQQ revisions (716.760009765625) are
observations, not attribution. Likewise the "dropped digit" reading of 660.65 (and of 283.92) is a hypothesis about the suspicious
value, not an established root cause; and the bank holding the earlier observation shows that the bank and the private tape diverged,
not that every correction was dropped rather than overwritten later. Identifying the adapter, the request and the write path needs
correlated producer/request/write evidence from the platform owners; until then the outcome is what is established: **two tapes**.
...
(b) the warm-up hash (`plan.warmup.hash`, F99) likewise; (c) the platform owners' correlated producer/request/write evidence on which adapter delivered these corrections and why the bank
and the private tape diverged (and whether a correction that moves a low 100 points on unchanged volume should be quarantined at intake
rather than merged). Added to `2026-09-16-c6-completion-plan.md` as step 1e.

## 4. Not done, on purpose

No frozen plan was edited, no bar was repaired, no decision was re-run. The suspicious lows are reported with their chain of
observations, the chronology kept apart from the hypotheses; adapter/provider and persistence cause are labelled unresolved; how the
bank and the private tape should reconcile is the platform owners' policy (PLATFORM-RULES entry added).
