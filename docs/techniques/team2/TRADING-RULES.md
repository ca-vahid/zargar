...
| Q2 size buckets | 1.0 / 0.5 / 0 | Is the middle bucket worth trading at all? | sweep expectancy per bucket, ≥ 60 sessions | 2026-09-13: `size_small=0` under the conjunction costs 300 pnl%-sum on 48 symbol-sessions — the middle rung carries the added trades; open on the canonical tape · 2026-09-15: the sizing-cap experiment (`size_full` 0.5, own Practice book `Team2 Sizing 0.5`, −$800 sampled threshold) is PREPARED and OFF pending C6 + GO |
| C1 no-trade zone | V6 picture (`no_trade_zone=pm_range`) | Is B5's conjunction the rule? | knob built OFF (v0.7.53); frozen sample 54→82 trades, 209.5→358.6 but concentrated in week 37; book-level earlier +$374 | research; activation after C6 + an approved Practice experiment · 2026-09-15: labelled experiment book `Team2 C1 Conjunction` PREPARED and OFF (follow-on to the sizing cap; −$1,000 policy threshold) |
...
- **2026-09-14 v0.7.80 acceptance (other team: zero-fill reconciliation ACCEPTED; one F item; v0.7.81)** — a terminal
  ...
  first (`_apply_entry_fill`, shared with the fill branch: never regresses, opens once) and only then classifies; the
  restore path inherits it through the same callback. Their packet `tests/test_codex_team2_v080_terminal_fill.py`
  verbatim (live + restart); zero-fill controls unchanged. No threshold or setting changed.
- **2026-09-14 v0.7.78 acceptance (other team: E and G ACCEPTED, F's initial protection ACCEPTED; one F reconciliation
  ...
  still-uncertain entry against the persisted ORDER row through the same callback (terminal or filled row = authoritative;
  in-flight or missing row keeps the uncertainty). Duplicate confirmations are idempotent; cancel-after-partial keeps the
  fill and is never exempt. Their packet `tests/test_codex_team2_v078_reconciliation.py` verbatim. No threshold or setting
  changed.
