...
| 11 | Every share observation EM ever recorded - 5 of 5 - was discarded as unscorable, while options from the same recorder scored normally | `_shadow_capture_rung` built its evidence from the raw `Quote` instead of the shared policy. `Quote.source` is `""` by contract on every equity, so the substitution its sibling recorders apply was never seen | Equities read `quote_ts`, then `last_ts`, then `source_ts`, and **never** the receipt. The substituted label carries `sourceBasis: engine_feed` so a reader can tell it apart from a venue identity, and `first_sale` persists that field instead of dropping it | 8 provenance tests; the reviewers' measurement-boundary suite caught an over-strict first version and the fallback order was widened rather than their fixture changed | Whether a Yahoo-routed symbol has ever been labelled `feed:HybridQuoteFeed` cannot be ruled out: the records do not say which leg of the hybrid feed served a quote |
| 12 | A missing venue stamp silently became the receipt time, so `ageS` measured how long ago **we** saw the quote | The `or oq.ts` fallback in the same block | Removed. A missing venue time stays missing | Test: "a venue time is never borrowed from the host clock" | None. The Tips desk raised the same point independently as a caveat the same afternoon |
