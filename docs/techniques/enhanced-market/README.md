...
- **Integrated delivery, corrected candidate `9ea7572e` (2026-09-19, 0.8.21, NOT deployed; candidate review IR-01..IR-05 answered): source scenarios + plan matcher, the preparation policy owner (baseline default / deterministic built), order-free source continuation + requalification with `candidate-pricing-v1`, `first-sale-v2` (default OFF; validated executable bound owns admission, enforce fails closed and is rechecked at final dispatch), the ED-04 `book-snapshot-v2` recorder + execution-backed ledger + reducer + report (OFF) and a read-only review panel on Validation. One closure (revision 2): `reviews/INTEGRATED-DELIVERY-RESPONSE-2026-09-18.md`; frozen definitions: `research/PROSPECTIVE-DEFINITIONS-2026-09-19.md`. Recommended order: deploy with everything off, then the ED-04 recorder, then first-sale OBSERVE; no enforcement on one historical loser. Baseline Practice preparation and trading unchanged; deployment and every activation are separate user decisions.**
