...
| **Venue capability check: short options + multi-leg on Webull CA via SnapTrade** | T5, covered calls, wheel | We have verified long options only. Need: SELL_TO_OPEN acceptance, spread/multi-leg support (SnapTrade mleg?), margin treatment. A `snaptrade_options_check --short` style probe, read-only first. **Gates the whole income family.** |
