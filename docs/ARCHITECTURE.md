...
| Techniques | `zargar/techniques/` (registry, tip, team2, flow, options_cartel) + `zargar/technique/` (EnhancedMarket) | `TechniqueInfo` registry (`GET /api/techniques`) lists five (2026-09-13): **EM** (`technique/` — rulebook, setups, plans, prompts/schemas, vision, chat, `arming.PlanArmer(PlanRunner)`), **Tip** (`techniques/tip/` — plan/horizon/express + `runner.TipRunner(PlanRunner)`; intake stays in `signals/`; dual shadow books via `Portfolio.book`; filled entries hand off to the PositionManager), **Flow** (`techniques/flow/` — pure scan math + `FlowService` on the scheduler; context-only, never orders; `FlowContextServed` journals every delivery), **Team2** (`techniques/team2/` — intraday plan/read/runner), and **Options Cartel** (`techniques/options_cartel/` — preparation, swing plans, execution policy and research). |
