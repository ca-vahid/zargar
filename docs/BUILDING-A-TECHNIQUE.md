...
| Halts | `engine.trading_halted(pid)`, `techniques.<id>.daily_loss_halt_pct` | Three scopes (PLATFORM-RULES 2026-09-04): the global switch, the per-book daily-loss halt (default scope), and your technique's own day-loss pause on a book — set the % in your DEFAULTS; the runner pauses your plans on that book when your realised + open loss crosses it. Ask `engine.trading_halted(portfolio_id)` before a fire; never read `halt.engaged` alone. |
...
| Bars beyond RTH (2026-09-03, Team2 desk) | `marketstructure.history.fetch_window(..., session="ext")`, `fetch_extended_session`, `marketstructure.aggregate` (1m→2m/5m/15m wall-clock buckets, `bar_session`, `filter_session`, `closed_bars`), `marketstructure.indicators` (EMA series/state, stack, fan), `marketstructure.dailylevels` (prior-day zones wick→next body, pre-market range), `marketstructure.market_calendar` (holidays, 13:00 early closes — `session_bounds` honours them), nightly `research.ext_bars` + `research.vix` jobs, `options.pick.select_by_premium` (premium-targeted strike), `engine.macro` (manual macro-event list; placeholder source) | Everything a 2-minute, extended-hours, 0DTE method needs. RTH-only callers see no change. A technique whose entry mechanics do not fit `TriggerTracker` may ship ONE pure session function used both live and in replay (Team2: `techniques/team2/session.py`) — parity by construction. 0DTE needs a per-technique policy `techniques.<id>.zero_dte` (RiskGate); without it the never-list rejects. |
