...
async def test_reserve_forces_a_terminal_answer_before_the_deadline(rig):
    """A scripted near-deadline tool chain: once the remaining budget drops inside the reserve
    the loop stops tool work, tells the model to answer with tools off, and a verdict lands."""
    eng = rig
    run_id = new_id()
    await _mk_run(eng, run_id)
    client = _SlowScripted([(0.9, _tool_use(1)), (0.9, _tool_use(2)), (0.1, _Resp([_Block(type="text", text=_json_opinion())]))])
    state = _arm_deadline({}, eng, timeout_s=4.0)
    state["reserveS"] = 2.5
    ...
    assert state["finalDemanded"] is True and state.get("failure") is None
...
async def test_intake_review_failing_after_a_note_keeps_one_receipt_and_a_typed_failure(rig, monkeypatch):
    ...
    client = _SlowScripted([
        ...
        (6.0, _Resp([_Block(type="text", text="never arrives in time")])),
    ])
    ...
    assert (op.get("usage") or {}).get("partial") is True and op["usage"]["unknownCalls"] == 1
