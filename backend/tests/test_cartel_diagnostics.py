...
@pytest.mark.asyncio
async def test_older_session_counter_is_persisted_without_new_current_day_drop():
    registry = DropRegistry(interval_ms=1)
    now = NEXT_OPEN + 30 * MIN
    registry.note('r1', NEXT_OPEN, now)
    saved = []
    async def persist(run_id, payload):
        saved.append(payload)
    await registry.flush(['r1'], now, persist)
    registry.note('r1', OPEN, now+2)
    await registry.flush(['r1'], now+2, persist)
    assert saved[-1]['olderSessionBars'] == 1
    assert saved[-1]['session'] == NEXT_DAY.isoformat()
