...
async def restart_state(engine) -> dict:
    ...
    if orders is not None:
        try:
            working_orders = [str(o.get("id")) for o in await orders.list_orders(open_only=True, limit=500)]
    ...
    return {
        ...
        "workingOrders": sorted(working_orders),
...
def readiness_from_state(state: dict) -> dict:
    """Safe = nothing a restart would interrupt. Armed plans and open managed positions are
    write-ahead and restore, so they are reported, not blocking; anything with an order in the
    air or money mid-decision blocks."""
    reasons: list[str] = []
    ...
    if state.get("workingOrders"):
        reasons.append(f"{len(state['workingOrders'])} working order(s) at the venue")
    ...
    if journal:
        try:
            ...
            await engine.journal.append(ev.OPS_RESTART_CHECK, {
                ...
                "workingOrders": len(out["state"]["workingOrders"]), "marketOpen": out["state"]["marketOpen"],
            })
...
def compare_states(before: dict, after: dict) -> dict:
    ...
    missing = {}
    for key in ("armed", "openTrades", "pendingExits", "workingOrders"):
