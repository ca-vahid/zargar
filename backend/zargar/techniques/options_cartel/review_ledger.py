...
def summarize_fills(rows, begins, cutoff, ownership=None):
    ...
    for fill, order in sorted(rows, key=lambda r: (r[0].ts, r[0].id)):
        ...
        asset = assets.setdefault(key, {'symbol': order.symbol, 'secType': order.sec_type, 'planId': key[2],
            ...
            'exitOrders': set(), 'executionIds': [], 'remainingQty': 0., 'entryFilledQty': 0., 'entryFilledQtyToday': 0.,
            'exitFilledQty': 0.})
        ...
        if fill.side == 'BUY':
            asset['entryFilledQty'] += fill.qty   # cumulative entry fills; holdings are NOT this number
            if today:
                asset['entryFilledQtyToday'] += fill.qty
            lots[key].append([fill.qty, fill.price, fill.commission/fill.qty, order.id])
        elif fill.side == 'SELL':
            asset['exitFilledQty'] += fill.qty
            left = fill.qty
