...
class SourcePolicy:
    ...
    budget_per_tip: float = 500.0     # max $ committed to one tip (option debit / share notional)
    budget_open_max: float = 2000.0   # max $ open across this source's tips at once
    dte_min: int = 10                 # option expression window (never 0DTE)
    dte_max: int = 30
    horizon_sessions: int = 10        # tip expires unfilled/unresolved after N sessions
...
def resolve_policy(settings, source_name: str | None) -> SourcePolicy:
    ...
    return SourcePolicy(
        ...
        budget_per_tip=float(pick("budget_per_tip", 500.0)),
        budget_open_max=float(pick("budget_open_max", 2000.0)),
        dte_min=int(pick("dte_min", 10)),
        dte_max=int(pick("dte_max", 30)),
        horizon_sessions=int(pick("horizon_sessions", 10)),
        min_conviction=str(pick("min_conviction", "implied")),
        max_open_tips=int(pick("max_open_tips", 3)),
    )
