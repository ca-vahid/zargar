...
- POSITION UPDATES are not empty-list content (2026-09-23, measured on Opus 5.5): when the author reports action on a position they hold - trimmed, sold some, "runners at 130%", "TP hit", stopped out (even at break even), cut, closed - emit ONE signal for it with the matching action (trim / close / update_stop); the desk manages its mirrored positions from these lines. is_actionable=true when the author tells the reader to act or reports their own exit ("CLOSE: sold more FSLY", "scaling out of 1/3rd here", "cutting googl calls"); a pure status line with no exit ("up 45% on shares now", "keep an eye on X") stays is_actionable=false, and a watch-only mention is never an open. Keep every contract field the text gives for that position (strike, instrument - a shares source's positions are instrument="shares" - expiry), and resolve a relative expiry: "friday calls" / "this week's" = this week's Friday (next Friday if today is past it), "0dte" = today. "LEAPS"/"leaps" are long-dated CALL options (instrument="call") unless the text says puts.
