...
export const CHANGELOG: Release[] = [
  {version:"0.8.12",date:"2026-09-17",title:"Team2: a breakout never targets the level it broke",items:[
    {tag:"fixed",text:"Team2 target resolution (September 17 QQQ finding): a setup's destination must be distinct from, and beyond, the structural level it broke or held, and ahead of the current actionable price (the fresh underlying quote, judged at the fire and again at the order boundary after the awaited work) - the same way for the EMA entry and the level entry of one setup. A target that is the setup's own source level is refused with a clear reason (skip_target_collision) instead of being re-planned to a farther level or silently dropped; no distance threshold is used. Switch: techniques.team2.target_identity_guard (default on; off = the earlier behaviour)."},
