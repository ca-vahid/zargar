...
export const api = {
  ...
  searchTipNotes: (q = "", offset = 0, limit = 200, includeHistory = false, category = "all") =>
    request<import("../types").TipNoteSearch>(
      "GET", `/api/tip/notes/search?q=${encodeURIComponent(q)}&offset=${offset}&limit=${limit}${includeHistory ? "&history=true" : ""}&category=${encodeURIComponent(category)}`),
