...
export function CartelSessionReview() {
  ...
  const time = (ms:number) => new Date(ms).toLocaleTimeString([], {timeZone:'America/New_York', hour:'2-digit', minute:'2-digit'});
  const blockerText = (b:any) => `${time(b.at)} · ${b.stage} · ${b.text || b.rule}`;
  const attribution = (a:any) => a && <div className="cartel-attribution">
    <p><strong>Actual outcome:</strong> {a.actualOutcome.replaceAll('_',' ')} · net {money(a.economics?.actualNetRealized)} · fees {money(a.economics?.actualFees)} · {a.economics?.actualBasis}</p>
    {a.priceTouch?.explanation && <p>{a.priceTouch.explanation}</p>}
    {a.firstKnownBlocker && <p><strong>First known blocker:</strong> {blockerText(a.firstKnownBlocker)}</p>}
    {a.otherIndependentBlockers?.length > 0 && <p><strong>Also independent:</strong> {a.otherIndependentBlockers.map((b:any) => `${blockerText(b)}${b.sameWindowAsFirst ? ' (same window)' : ''}`).join(' · ')}</p>}
    {a.incompleteWindows?.length > 0 && <p className="muted">{a.incompleteWindows.length} incomplete window(s){a.earlierUnknownWindows ? `, ${a.earlierUnknownWindows} before the first known blocker` : ''}: {a.incompleteWindows.map((w:any) => time(w.at)).join(', ')}. These remain unknown; recovered data never authorises a past order.</p>}
    <p className="muted">Contract search: {a.selectionCoverage?.status.replaceAll('_',' ')} · {a.selectionCoverage?.basis}{a.selectionCoverage?.selectionVersion ? ` · ${a.selectionCoverage.selectionVersion}` : ''}</p>
  </div>;
  const cadenceLine = (c:any) => c?.control && <p className="muted">Cadence {c.executing || 'unlabelled'} ({c.executingTimeframeMinutes}m) executing · matched {c.control.control} control: {c.control.controlSignals} confirmation(s){c.control.controlFirstSignalAt ? ` (first ${time(c.control.controlFirstSignalAt)})` : ''}; control places no orders and has no P&L.</p>;
  return <details className="cartel-inset"><summary>Daily review · trades, costs and missed opportunities</summary>
    ...
    {data && data.reviewScope === scope && <><h3>{data.account} · {data.session}</h3>
      ...
      {data.cadenceComparison?.control && <p className="muted">Cadence comparison · executing {Object.entries(data.cadenceComparison.executing).map(([label, v]:[string, any]) => `${label}: ${v.plans} plans, ${v.signals} confirmations, ${v.entryOrders} entry orders, ${v.filledEntries} filled, net ${money(v.netRealized)}`).join(' · ')} · matched {data.cadenceComparison.control.cadence} control: {data.cadenceComparison.control.signals} confirmations on {data.cadenceComparison.control.plansWithSignal} of {data.cadenceComparison.control.plans} plans, no orders, no P&L.</p>}
      <div className="scroll-x"><table className="tbl"><thead><tr><th>Stock / plan</th><th>Outcome</th><th>Net realized</th><th>Remaining</th></tr></thead><tbody>
        {data.rows.map((r:any) => <tr key={r.planId}><td><CartelRunLink id={r.planId} onOpen={openRun}>{r.symbol}</CartelRunLink></td>
          <td>{r.category.replaceAll('_',' ')}{r.opportunity && <><p>{r.opportunity.summary}</p><p className="muted">Trigger {r.opportunity.trigger} · recorded high {r.opportunity.high ?? 'unknown'} · low {r.opportunity.low ?? 'unknown'} · {r.opportunity.unresolvedMinutes} unresolved minutes at the final cutoff (decision-time coverage is listed with each incomplete window).</p></>}{attribution(r.attribution)}{cadenceLine(r.cadence)}{r.category === 'execution_rejected' && !r.attribution &&<p>{r.latestExecutionCheck?.reasons?.join(' · ')}</p>}{r.exitReasons?.length > 0 && <p>{r.exitReasons.map(exitLabel).join(' · ')}</p>}<details><summary>Decision evidence · historical checks</summary>{r.executionChecks?.map((c:any,i:number) => <p key={`execution-${i}`}>{new Date(c.at).toLocaleTimeString()} · Entry check {c.passed ? 'passed' : 'refused'} · {c.reasons?.join(' · ')}{c.expression && ` · ${c.expression.symbol}: bid ${c.expression.bid}, ask ${c.expression.ask}`}</p>)}{r.decisions.map((d:any,i:number) => <p key={i}>{new Date(d.at).toLocaleTimeString()} · {d.reason}</p>)}<p>{r.recoveries?.length || 0} session recovery records</p></details></td>
