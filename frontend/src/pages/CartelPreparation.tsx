    ...
    {view === "settings" && config && <form className="panel-body cartel-form" onSubmit={e => { e.preventDefault(); void act(true); }}>
      ...
      <details open><summary>Market coverage and method choices</summary>
        ...
        <div className="cartel-fields">
          ...
          <label>Entry cadence (versioned)<select aria-label="Entry cadence" value={config.entryCadence || (config.entry.timeframe_minutes === 15 ? "breakout_15m_v1" : "legacy_timeframe")} disabled={live}
              onChange={e => {const cadence = e.target.value; const minutes = cadence === "breakout_5m_v1" ? 5 : cadence === "breakout_15m_v1" ? 15 : config.entry.timeframe_minutes; setConfig({...config, entryCadence:cadence, entry:{...config.entry, timeframe_minutes:minutes}});}}>
            <option value="breakout_15m_v1">breakout_15m_v1 — 15-minute confirmation (incumbent)</option>{!live && <option value="breakout_5m_v1">breakout_5m_v1 — 5-minute confirmation · Practice experiment, 15m matched control</option>}
            {(config.entry.timeframe_minutes === 30 || config.entryCadence === "legacy_timeframe") && <option value="legacy_timeframe">legacy_timeframe — saved unlabelled {config.entry.timeframe_minutes}-minute entry (no control)</option>}
          </select></label>
          <p>{live ? "Live keeps the 15-minute cadence." : "breakout_5m_v1 executes 5-minute confirmations on NEW long Practice plans only, with the same volume, close-quality, target and risk rules and its own 5-minute historical baseline; a bearish plan keeps the 15-minute cadence. Each long arm also records a non-ordering 15-minute matched control on the same tape that keeps observing through the whole session. Existing arms and positions keep the cadence they were prepared under; a saved unlabelled 5-minute entry is legacy_timeframe (no control, no pilot)."}</p>
          <label>Confirmation timeframe<select value={config.entry.timeframe_minutes} onChange={e => {const minutes = Number(e.target.value); setConfig({...config, entry:{...config.entry, timeframe_minutes:minutes}, entryCadence: minutes === 15 ? "breakout_15m_v1" : minutes === 5 && !live ? "breakout_5m_v1" : "legacy_timeframe"});}}>
          ...
          {!live && <>
          <label>Gap opens beyond the trigger<select aria-label="Gap opens" value={config.entry.gap_policy || "none"} onChange={e => setConfig({...config, entry:{...config.entry, gap_policy:e.target.value}})}><option value="none">No entry (breakout needs a crossing)</option><option value="retest_v1">retest_v1: enter on a completed retest candle · Practice experiment</option></select></label>
          <label>Volume dry-up rule<select aria-label="Volume dry-up rule" value={config.setups?.dry_up_rule || "ratio_v1"} onChange={e => setConfig({...config, setups:{...config.setups, dry_up_rule:e.target.value}})}><option value="ratio_v1">ratio_v1: base volume at most 0.8x the prior base</option><option value="non_increasing_v1">non_increasing_v1: base volume not above the prior base · Practice experiment</option></select></label>
          <label>Minimum first-target R to arm (0 = off)<input type="number" min={0} max={10} step="any" value={config.minArmTargetR ?? 0} onChange={e => setConfig({...config, minArmTargetR:Number(e.target.value)})}/></label>
          <label>Contract ranking<select aria-label="Contract ranking" value={config.contractPolicy?.rankingVersion || "legacy"} onChange={e => setConfig({...config, contractPolicy:{...config.contractPolicy, rankingVersion:e.target.value}})}><option value="legacy">legacy: nearest reviewed DTE, then delta</option><option value="executable_cost_v2">executable_cost_v2: lowest crossing cost within |delta| 0.35-0.65</option><option value="executable_cost_v1">executable_cost_v1: lowest cost at any delta (prefers deep in-the-money; not recommended)</option></select></label>
          <label>Contract search<select aria-label="Contract search" value={config.contractPolicy?.selectionVersion || "legacy"} onChange={e => setConfig({...config, contractPolicy:{...config.contractPolicy, selectionVersion:e.target.value}})}><option value="legacy">legacy: refresh the rows nearest the reviewed DTE</option><option value="diverse_liquidity_v1">diverse_liquidity_v1: saved contract first, spread across expiries</option></select></label>
          </>}
