    ...
    {view === "settings" && config && <form className="panel-body cartel-form" onSubmit={e => { e.preventDefault(); void act(true); }}>
      ...
      <details open><summary>Market coverage and method choices</summary>
        ...
        <div className="cartel-fields">
          ...
          <label>Entry cadence (versioned)<select aria-label="Entry cadence" value={config.entryCadence || (config.entry.timeframe_minutes === 5 ? "breakout_5m_v1" : config.entry.timeframe_minutes === 15 ? "breakout_15m_v1" : "legacy_timeframe")} disabled={live}
            ...
            {config.entry.timeframe_minutes === 30 && <option value="legacy_timeframe">legacy_timeframe — 30-minute research variant</option>}
          </select></label>
          <p>{live ? "Live keeps the 15-minute cadence." : "breakout_5m_v1 executes 5-minute confirmations on NEW Practice plans only, with the same volume, close-quality, target and risk rules and its own 5-minute historical baseline. Each arm also records a non-ordering 15-minute matched control on the same tape for the daily review. Existing arms and positions keep the cadence they were prepared under."}</p>
          <label>Confirmation timeframe<select value={config.entry.timeframe_minutes} onChange={e => {const minutes = Number(e.target.value); setConfig({...config, entry:{...config.entry, timeframe_minutes:minutes}, entryCadence: minutes === 5 ? "breakout_5m_v1" : minutes === 15 ? "breakout_15m_v1" : "legacy_timeframe"});}}>
