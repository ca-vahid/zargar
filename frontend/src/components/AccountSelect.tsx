...
export function AccountSelect({
  ...
  // keep an EXISTING selection inside the workspace; an empty value means "ask me"
  // (user 2026-09-07: one Practice book per technique - never guess which book a
  // manual trade belongs to)
  useEffect(() => {
    if (!scoped.length || !value) return;
  ...
  const selected = scoped.find((o) => o.portfolio.id === value);
    ...
    <div className="acct-select" ref={rootRef}>
      <button type="button" className="acct-btn" onClick={() => setOpen((v) => !v)}
        ...
        {selected
          ...
          : <span className="acct-opt-main"><span className="acct-opt-name">Choose an account</span>
              <span className="acct-opt-cash">which book should this trade go in?</span></span>}
        <IconChevron size={11}
