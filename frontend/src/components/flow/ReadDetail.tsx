...
import { Occ, fmtPrem, leanColor, leanPill, maxRepeat } from "./lib";
      ...
      <div className="flow-detail-body">
        ...
        <div className="flow-detail-duo">
          <div className="flow-mini-card">
            ...
            {maxRepeat(read) > 0 && (
              <div className="muted" style={{ fontSize: "var(--fs-0)", marginTop: 4 }}>
                hot: {Object.entries(read.repeatHits).map(([c, n], i) => (
                  <span key={c}>{i > 0 && ", "}<Occ contract={c} /> ×{n}</span>
                ))}
              </div>
            )}
          </div>
        ...
        <div className="flow-actions">
          <SendToTipsButton symbol={read.symbol} lean={read.lean} />
