    ...
    for (const t of a.triggers ?? []) {
      if (style === "zones") {
        ...
        plotLines.push({ value: t.entry, color: accent, width: 1.2, zIndex: 4,
          label: { text: `${trigWord(t)} fires ${fmt(t.entry)}`, align: "right", style: { color: accent, fontSize: "10px", fontWeight: "600" } } });
        ...
        plotLines.push({ value: t.entry, color: accent, width: 1.2, zIndex: 4,
          label: { text: `${trigWord(t)} fires ${fmt(t.entry)}`, align: "left", style: { color: accent, fontSize: "10px", fontWeight: "600" } } });
  ...
  return (
    <div className="tq-armed-day">
      <div className="tq-armed-day-now">
        <b>Now:</b>{" "}
        {waiting.length
          ? waiting.map((t: any) => <span key={t.id}><span className="tq-chip" title={t.id}>{trigWord(t)} @ {fmt(t.entry)}</span> {waitingFor(t, a.sessionWindowNow)}{t.distancePct !== undefined ? ` · ${t.distancePct > 0 ? "+" : ""}${t.distancePct.toFixed(2)}% away` : ""}. </span>)
          : <span>{a.summary}</span>}
      </div>
      {!sessionStarted && (
