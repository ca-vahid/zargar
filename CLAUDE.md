...
**EM stop rule + scorecard (2026-09-22, user decision):** `technique/em_scorecard.py` / `python -m zargar.tools.em_scorecard` - after 20 evaluable baseline sessions (from 2026-09-22) with cumulative R <= 0 and bootstrap upper mean R < +0.1R the paid review stops (`techniques.enhanced_market.paid_review` false, a human step); preregistered tests with fixed samples (shares fallback, short puts prime, stop vs volatility, one-touch levels, rules vs model) - a partial reading is never a verdict, don't tune those knobs before a test is ready. Recurring prep: `scripts/em-evening-batch.py` + `scripts/em-install-session-checks.ps1`. `TechniqueExitQuote` records each EM exit's decision quote. A US share class (BRK.B) streams from Alpaca (`is_us_share_class`).
**EM fully deterministic (2026-09-23, user decision, v0.8.37):** `technique/em_deterministic_prep.py` prepares the BASELINE book from the graded sheet by rules (zero model calls) once `techniques.enhanced_market.preparation_policy=deterministic`; the paid evening batch stands down (`paid_review=false`). Rollback = both keys back. Only author-note ingestion still calls a model automatically.
...
- **Every order goes through `RiskGate.evaluate()`** — no code path may submit
  ...
  reduce exposure). Kill-switch state must always be honored — in all four scopes
  (global switch, per-book daily-loss halt, per-technique day-loss pause, per-book PAUSE that
  survives restart and day rollover; PLATFORM-RULES 2026-09-04 + 2026-09-15): runners ask `engine.trading_halted(portfolio_id)`, never `halt.engaged` alone.
