...
| options | `pick`, `result`, `skipped` | `service.option_pick` → `options.CboeClient` / `TradierClient`, `pick_for_setup` | `technique.options.enabled`, `options.provider`, env `ZARGAR_TRADIER_TOKEN` | CBOE is US-only (`.TO`/`.V` → no chain); delayed quotes |
...
| review | `technique_reviews` rows, journal `TechniqueReviewAdded` | `service.add_review`, `review.py` taxonomy; chat tools `get_run` / `record_review` (`tools.py`) — the UI has no review form, reviews come from chat / CLI / this skill | — | — |
...
| arming | journal `TechniquePlanArmed/Paused/Resumed/Disarmed/TriggerFired/TriggerSkipped/OrderIntent/OrderResult/PositionOpened/Exit/PositionClosed/Error`, chat note `plan_trigger`, table `technique_armed`; `GET /api/technique/armed/{id}/audit` | `arming.PlanArmer` (bus `BARS` 1m + `ORDERS`), `ArmConfig` (account, mode alert/proposal/auto, risk %, max qty), `VisionPipeline.run_critic`, `_persist_setup` / `_emit_proposal` / `OrderManager.place` | `technique.arm.*` (`mode`, `default_portfolio`, `risk_pct`, `max_qty`, `allow_live_auto`, `slippage_pct`, `flatten_minutes_before_close`, `max_retries`, `stale_seconds`) | trigger fired mid-day (should be observed only); critic killed a good fire; entry REJECTED_RISK (collar / notional / stale quote); entry never filled (slippage too tight); exit late (closed-bar evaluation); stale bars (no fire); wrong account / mode on the card |
